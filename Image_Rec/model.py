...
def predict_image_week_9(image, model, signal):
    name_to_id = {
            "NA": 'NA',
            "Bullseye": 10,
            "One": 11,
    ...
    res = dict((v,k) for k,v in name_to_id.items())
    # Load the image
    img = Image.open(os.path.join('C:\\Users\\sanja\\OneDrive\\Desktop\\rpi_upload', image))
    # Run inference
    save_dir = "C:\\Users\\sanja\\OneDrive\\Desktop\\rpi_results"
    results = model(img, save=True , half=True, project = save_dir, name = "test", exist_ok=True)
    ...
    clss = list()
    for cno in clist:
        clss.append(model.names[int(cno)])

    image_id = None

    if len(clss) == 1:
        image_id = str(clss[0][-2:])
    else:
        if signal == "L":
            image_id = str(clss[0][-2:])
        
        elif signal == "R":
            image_id = str(clss[-1][-2:])
        else:
            image_id = str(clss[len(clss)//2][-2:])

    # elif len(results) > 1:
    #     # get the signal - it means there might be 
    #     # if the signal is 'L', choose the first prediction in the list, i.e. leftmost in the image
    #     if signal == 'L':
    #         clss_r = results[0]
    #         # print that we select the first one
    #         print(f'We choose the leftmost image for recognition')
    #     # if the signal is 'R', choose the last prediction in the list, i.e. rightmost in the image
    #     elif signal == 'R':
    #         clss_r = results[-1]
    #         print('We choose the rightmost image for recognition ')
    #     # if the signal is 'C', choose the prediction that is central in the image
    #     else:
    #         clss_r = results[len(results)//2]
    #         order = len(results)//2
    #         print(f'We choose the {order}th for the recognition (0-starting order)')

        # then do the same as above
        # clss_r.save('runs')
        # clist= clss_r.boxes.cls
        # clss = list()
        # for cno in clist:
        #     clss.append(model.names[int(cno)])
    # else: # the thing 0
    ...
    # print(res[int(clss[0][-2:])])
    # Convert the results to a dataframe
    # df_results = results[0].pandas().xyxy[0]
...
def stitch_image():
    ...
    imgFolder = 'C:\\Users\\sanja\\OneDrive\\Desktop\\rpi_results\\stitched'
    stitchedPath = os.path.join(imgFolder, f'stitched-{int(time.time())}.jpeg')

    # Find all files that ends with ".jpg" (this won't match the stitched images as we name them ".jpeg")
    imgPaths = glob.glob(r'C:\Users\sanja\OneDrive\Desktop\rpi_results\test\*.jpg')
    # Open all images
